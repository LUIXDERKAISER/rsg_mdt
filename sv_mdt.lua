...
AddEventHandler("ghost_mdt:performOffenderSearch", function(query)
	...
	exports.ghmattimysql:execute("SELECT * FROM `charinfo` WHERE LOWER(`firstname`) LIKE @query OR LOWER(`lastname`) LIKE @query OR CONCAT(LOWER(`firstname`), ' ', LOWER(`lastname`)) LIKE @query", {
...
AddEventHandler("ghost_mdt:getOffenderDetailsById", function(char_id)
    ...
    exports.ghmattimysql:execute('SELECT * FROM `charinfo` WHERE `citizenid` = ?', {char_id}, function(result)
